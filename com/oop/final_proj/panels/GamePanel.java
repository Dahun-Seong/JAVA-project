/*** 젤리(Jelly)를 과제(Assignment)로 변경했다. ***/

package com.oop.final_proj.panels;

import com.oop.final_proj.ingame.Assignment;
import com.oop.final_proj.ingame.Back;
import com.oop.final_proj.ingame.Field;
import com.oop.final_proj.ingame.Cookie;
import com.oop.final_proj.ingame.MapObjectImg;
import com.oop.final_proj.ingame.Tacle;

// 성다훈 : audio
import com.oop.final_proj.ingame.Audio;
import com.oop.final_proj.panels.IntroPanel;

import com.oop.final_proj.main.Main;
import com.oop.final_proj.ingame.CookieImg;
import com.oop.final_proj.util.GameUtil;
...
public class GamePanel extends JPanel {
    ...
    // 과제 이미지 아이콘들
    ImageIcon jelly1Ic, jelly2Ic, jelly3Ic, jellyHPIc;
    ImageIcon jellyEffectIc;

    // 발판 이미지 아이콘들
    ImageIcon field1Ic, field2Ic;
    ...
    ImageIcon redBg; // 피격시 붉은 화면

    //ImageIcon jumpButtonIconUp, jumpButtonIconDown;
    //ImageIcon slideIconUp, slideIconDown;
    //Image jumpBtn, slideBtn;

    // 리스트 생성
    ArrayList<Assignment> assignList; // 과제 리스트
    ArrayList<Field> fieldList; // 발판 리스트
    ArrayList<Tacle> tacleList; // 장애물 리스트
    ArrayList<Integer> mapLengthList;
    ...
    boolean escKeyOn = false; // 일시정지를 위한 esc키 확인
    ...
    Color backFade; // 배경이 바뀔 때 fade-out & fade-in 하기 위한 컬러변수

    MapObjectImg mo1, mo2, mo3, mo4;  // 맵 오브젝트의 이미지들
    ...
    public void gameSet(CookieImg ci) {
        setFocusable(true);  //현재 컴포넌트(CookieImg 객체??)에 포커스를 집중함.

        initCookieImg(ci); // 쿠키 이미지 생성
    ...
    /* 메인에서 받은 쿠키 이미지 아이콘들을 인스턴스화하는 메소드 */
    ...
    private void initObject() {
        ...
        assignList = new ArrayList<>(); // 과제 리스트
        fieldList = new ArrayList<>(); // 발판 리스트
        tacleList = new ArrayList<>(); // 장애물 리스트
        mapLengthList = new ArrayList<>(); // 다음 맵의 시작지점을 확인하기 위한 배열

        makeMo();  // 맵 인스턴스들을 생성 (mo1, mo2, mo3, mo4 생성)
        ...
        face = c1.getX() + c1.getWidth();  // 쿠키의 정면 위치 (쿠키의 x값과 너비를 더한 값)
        foot = c1.getY() + c1.getHeight();  // 쿠키의 발밑 위치 (쿠키의 y값과 높이를 더한 값)
        ...
        /*** 하나의 배경 이미지로 흐르는 배경화면 효과를 내기 위해
         *   동일한 이미지를 하나는 (0,0)에 배치하고
         *   다른 하나는 바로 뒤따라오도록 ('배경사진의 너비',0)에 배치했다. ***/
    ...
    private void makeMo() {
        ...
        mo1 = new MapObjectImg(new ImageIcon("img/Objectimg/map1img/bg1.png"),
                new ImageIcon("img/Objectimg/map1img/bg2.png"), new ImageIcon("img/Objectimg/map1img/book.png"),
                new ImageIcon("img/Objectimg/map1img/book.png"), new ImageIcon("img/Objectimg/map1img/book.png"),
                new ImageIcon("img/Objectimg/map1img/life.png"), new ImageIcon("img/Objectimg/map1img/effectTest.png"),
                new ImageIcon("img/Objectimg/map1img/fieldIc1.png"),
                new ImageIcon("img/Objectimg/map1img/fieldIc2.png"), new ImageIcon("img/Objectimg/map1img/tacle1.gif"),
                new ImageIcon("img/Objectimg/map1img/tacle2.png"), new ImageIcon("img/Objectimg/map1img/tacle3.png"),
    ...
    /*** 우리 프로젝트에 이용한다면... 게임 맵을 직접 만들어야 할 듯...? 아니면 그대로 사용? ***/
    /*** 분석하다 말았다... ***/
    ...
    private void initMap(int num, int mapLength) {
        ...
        // 맵 정보 불러오기
        try {
            sizeArr = GameUtil.getSize(tempMap); // 맵 사이즈를 배열에 저장
            colorArr = GameUtil.getPic(tempMap); // 맵 픽셀값을 배열에 저장
        } catch (Exception e1) {
            e1.printStackTrace();
        }

        tempMapLength = sizeArr[0];  //맵 내부에서 현재의 위치(길이)(?)
        int maxX = sizeArr[0]; // 맵의 넓이
        int maxY = sizeArr[1]; // 맵의 높이


        /*** 단순화 or 삭제 가능 코드 ***/
        for (int i = 0; i < maxX; i++) { // 과제는 1칸을 차지하기 때문에 1,1 사이즈로 반복문을 돌린다.
            for (int j = 0; j < maxY; j++) {
                if (colorArr[i][j] == 16776960) { // 색값이 16776960일 경우 기본젤리 생성 (먹을 시 점수 1234점 상승)
                    // 좌표에 40을 곱하고, 넓이와 높이는 30으로 한다.
                    assignList.add(new Assignment(jelly1Ic.getImage(), i * 40 + mapLength * 40, j * 40, 30, 30, 255, 1234));

                } else if (colorArr[i][j] == 13158400) { // 색값이 13158400일 경우 노란젤리 생성 (먹을 시 점수 2345점 상승)
                    // 좌표에 40을 곱하고, 넓이와 높이는 30으로 한다.
                    assignList.add(new Assignment(jelly2Ic.getImage(), i * 40 + mapLength * 40, j * 40, 30, 30, 255, 2345));

                } else if (colorArr[i][j] == 9868800) { // 색값이 9868800일 경우 핑크젤리 생성 (먹을 시 점수 3456점 상승)
                    // 좌표에 40을 곱하고, 넓이와 높이는 30으로 한다.
                    assignList.add(new Assignment(jelly3Ic.getImage(), i * 40 + mapLength * 40, j * 40, 30, 30, 255, 3456));

                } else if (colorArr[i][j] == 16737280) { // 색값이 16737280일 경우 물약젤리 생성 (먹을 시 점수 4567점 상승)
                    // 좌표에 40을 곱하고, 넓이와 높이는 30으로 한다.
                    assignList.add(new Assignment(jellyHPIc.getImage(), i * 40 + mapLength * 40, j * 40, 30, 30, 255, 4567));
                }
            }
        }
    ...
    private void initListener() {
        addKeyListener(new KeyAdapter() { // 키 리스너 추가
            ...
            public void keyPressed(KeyEvent e) {
                ...
                if (!escKeyOn) {  //ESC키를 누르지 않았을 때
                    if (e.getKeyCode() == KeyEvent.VK_SPACE) {  // 스페이스 키를 눌렀을 때
                        //jumpBtn = jumpButtonIconDown.getImage();  //삭제 가능
                        if (c1.getCountJump() < 2) {  // 더블점프가 2가 아닐때
                            jump(); // 점프 메서드 가동
                        }
                    }
            ...
            public void keyReleased(KeyEvent e) {
                if (e.getKeyCode() == KeyEvent.VK_DOWN) { // 다운키를 땠을 때
                    //slideBtn = slideIconUp.getImage();
                ...
                if (e.getKeyCode() == KeyEvent.VK_SPACE) {  //스페이스 키를 땠을 때
                    //jumpBtn = jumpButtonIconUp.getImage();
                }
            }
        });
    ...
    public void gameStart() {
        mapMove(); // 배경, 과제, 발판, 장애물 작동
        fall(); // 낙하 스레드 발동
    }


    /* 화면을 움직이고 과제를 먹거나, 장애물에 부딛히는 등의 이벤트를 발생시키는 메서드 */
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                while (true) {
                    ...
                    //(캐릭터의 발이 화면 아래로 떨어지거나?)하거나 체력이 다 떨어진 경우 게임 종료 처리
                    ...
                    // 배경 이미지 변경
                    if (fadeOn == false) { // fade-out이 아닐 때
                    ...
                    if (b21.getX() < -(b21.getWidth() - 1)) { // 배경2-1 이 -(배경넓이)보다 작으면, 즉 화면밖으로 모두나가면 배경 2-2뒤에 붙음
                        b21.setX(b21.getWidth());
                    }
                    if (b22.getX() < -(b22.getWidth() - 1)) { // 배경2-2 가 -(배경넓이)보다 작으면, 즉 화면밖으로 모두나가면 배경 2-1뒤에 붙음
                    ...
                    for (int i = 0; i < assignList.size(); i++) {
                        Assignment tempJelly = assignList.get(i); // 임시 변수에 리스트 안에 있는 개별 젤리를 불러오자
                            ...
                            if (c1.getImage() != slideIc.getImage()
                                ...
                                if (tempJelly.getImage() == jellyHPIc.getImage()) {
                                    if ((c1.getHealth() + 100) > 1000) {  //이미 체력이 많은 경우
                                        ...
                                        c1.setHealth(c1.getHealth() + 100);  //디폴트로는 체력 100씩 상승
                                    }
                                }
            ...
            public void run() {
                while (true) {
                    ...
                    if (!escKeyOn // 일시중지가 발동 안됐을 때
                        ...
                        long t1 = GameUtil.getTime(); // 현재시간을 가져온다
                        long t2;
                        int set = 1; // 처음 낙하량 (0~10) 까지 테스트해보자

                        while (foot < nowField) { // 발이 발판에 닿기 전까지 반복

                            t2 = GameUtil.getTime() - t1; // 지금 시간에서 t1을 뺀다
                            ...
                            if (escKeyOn) {
                                ...
                                tempT2 = GameUtil.getTime() - tempT1;
                                t1 = t1 + tempT2;
                            }
    ...
    protected void paintComponent(Graphics g) {
        // double-buffering : 그림을 미리 그려놓고 화면에 출력하므로 이미지의 깜빡거림을 해결하는 기법.
        // 버퍼 역할을 해줄 메모리 장치 컨텍스트(보이지 않는 화면)를 하나 더 사용하여
        // 그곳에 이미지를 그리고, 기존 화면을 유지하다가 이미지가 완성되면
        // 실제 화면 장체 컨텍스트로 한꺼번에 베껴 그리는 것이다.
        ...
        if (buffg == null) {
            ...
            if (buffImage == null) {
                System.out.println("double-buffering용 off-screen 생성 실패");
            } else {
                buffg = buffImage.getGraphics();  //버퍼의 그래픽 객체 얻기
            }
        }

        Graphics2D g2 = (Graphics2D) buffg;  // 투명도 관련
        ...
        // 맵 넘어갈 시 fade-out & fade-in 효과
        if (fadeOn) {
            buffg.setColor(backFade); // 투명하게 하는 방법 1
            buffg.fillRect(0, 0, this.getWidth(), this.getHeight());
        }

        // 발판을 그린다
        for (int i = 0; i < fieldList.size(); i++) {
            Field tempFoot = fieldList.get(i); //리스트 내 요소들을 인스턴스화
        ...
        // 젤리를 그린다
        for (int i = 0; i < assignList.size(); i++) {
            Assignment tempJelly = assignList.get(i);

            if (tempJelly.getX() > -90 && tempJelly.getX() < 810) {  //(-90,810) 범위 내의 젤리들만
                alphaComposite = AlphaComposite.getInstance(AlphaComposite.SRC_OVER,
                        (float) tempJelly.getAlpha() / 255);
                g2.setComposite(alphaComposite); // 투명하게 하는 방법 2 → 젤리를 그리기 전에 먼저 화면을 투명하게 한 이유?

                buffg.drawImage(tempJelly.getImage(), tempJelly.getX(), tempJelly.getY(), tempJelly.getWidth(),
                        tempJelly.getHeight(), null);  //젤리를 그린다.
        ...
        if (c1.isInvincible()) { // 피격 무적 상태일 경우
            // 쿠키의 alpha값을 받아온다
...
//		buffg.drawString(Integer.toString(resultScore), 700, 85);
        ...
        // 점수를 그린다
        GameUtil.drawFancyString(g2, Integer.toString(resultScore), 600, 58, 30, Color.WHITE);
